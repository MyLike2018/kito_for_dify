    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            if sandbox is not None:
                stop_mcpserver = "pkill -9 mcp-proxy"
                response = sandbox.process.exec(command=stop_mcpserver,cwd="/workspace")

                if int(response.exit_code) == 0:
                    task_output = f"success: stop mcpserver:{response.result}" 
                else:
                    task_output = f"error: stop mcpserver failed"
            ...
            task_output= f"error: stop mcpserver error, {str(e)}"
